...
function format_queue()
{
    jbg_format_in="$@"
    jbg_format_array=(${jbg_format_in// / })
        ...
        do
            if [[ ! -z "${line// }" ]];then
                ...
                if (( nline > 2 ));then
                    ...
                    if [[ "$outType" == "all" ]]; then
                        if (( nline == 3 ));then
                            echo -e "$strStart $_jbg_set_mark_job_wait " "$(format_queue $line)"; 
                        else
                            strStatus=$(echo "$line" | awk '{print $5}')
                            if [[ "$strStatus" == 'RUN' ]] || [[ "$strStatus" == 'R' ]]; then 
                                echo -e "$strStart""\033[$_jbg_set_color_job_run $_jbg_set_mark_job_run \033[0m" "$(format_queue $line)";
                                n_run=$((n_run+1)) ;
                            elif [[ "$strStatus" == 'PD' ]] || [[ "$strStatus" == 'PEND' ]] || [[ "$strStatus" == 'H' ]]  || [[ "$strStatus" == 'Q' ]]; then
                                echo -e "$strStart""\033[$_jbg_set_color_job_wait $_jbg_set_mark_job_wait \033[0m" "$(format_queue $line)"; 
                                n_wait=$((n_cg+1)) ;
                            else
                                # [[ "$strStatus" == 'CG' ]]; then
                                ...
                                echo -e "$strStart""\033[$_jbg_set_color_job_warn $_jbg_set_mark_job_warn \033[0m" "$(format_queue $line)"; 
                                n_cg=$((n_cg+1)) ;
                            fi
                            n_job=$((n_job+1));
                        fi
...
if [[ ! -z "$(_jbg_fuc_exist _jobage_queue_history_display)" ]]; then
    _jobage_cd()
    {
        if [[ "$_jbg_SHELL" ==  *"/bash" ]]; then
            if [ "$#" -eq 0 ]; then
...
if [[ ! -z "$(_jbg_fuc_exist _jobage_cancel_index)" ]]; then
    _jobage_kill_grep() {
...
if [[ ! -z "$(_jbg_fuc_exist _jobage_save_queue)" ]] && [[ ! -z "$(_jbg_fuc_exist _jobage_queue_display)" ]]; then
    jbg.q() 
    {
        if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
...
if [[ ! -z "$(_jbg_fuc_exist _jobage_kill_grep)" ]] && [[ ! -z "$(_jbg_fuc_exist _jobage_cancel_all)" ]] && [[ ! -z "$(_jbg_fuc_exist _jobage_cancel_index)" ]]; then
    jbg.kill() {
        if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
...
if [[ ! -z "$(_jbg_fuc_exist _jobage_cd)" ]]; then
    jbg.cd()
    {
        if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
...
if [[ ! -z "$(_jbg_fuc_exist _jobage_submit)" ]]; then
    jbg.sub()
    {
        if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
...
jbg.help() 
{
    ...
    echo '| command list :'
    if [[ ! -z "$(_jbg_fuc_exist jbg.q)" ]]; then
        echo '| .q      | display queue infomation.'
    fi
    if [[ ! -z "$(_jbg_fuc_exist jbg.qrun)" ]]; then
        echo '| .qrun   | display running queue infomation.'
    fi    
    if [[ ! -z "$(_jbg_fuc_exist jbg.qh)" ]]; then
        echo '| .qh     | display last two queue infomation.'
    fi 
    echo '| - - - - ' 
    if [[ ! -z "$(_jbg_fuc_exist jbg.sub)" ]]; then
        echo '| .sub    | submit job.'
    fi      
    if [[ ! -z "$(_jbg_fuc_exist jbg.kill)" ]]; then
